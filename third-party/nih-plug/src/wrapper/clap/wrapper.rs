...
impl<P: ClapPlugin> Wrapper<P> {
    ...
    ///
    /// Implicitly emits parameter value notifications for the editor and host.
    ///
    /// # Notes
    ///
    ...
    unsafe extern "C" fn ext_state_load(
        ...
        let Ok(length) = usize::try_from(length) else {
            nih_debug_assert_failure!("State length does not fit in memory on this platform.");
            return false;
        };

        let mut read_buffer: Vec<u8> = Vec::new();
        if read_buffer.try_reserve_exact(length).is_err() {
            nih_debug_assert_failure!("Could not reserve memory for a {} byte state.", length);
            return false;
        }
